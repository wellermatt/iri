...
res=rbindlist(lapply(res.files,function(x) {
    ...
    data.table(method = vars[1], freq = vars[2], freq.cycle = vars[3],f_results.load(x))
}))[freq.cycle!=52]

dcast(res, fc.item~method+freq+freq.cycle)
dcast(res, freq+lvl~method,fun.aggregate= median,na.rm=TRUE, value.var="ape")
